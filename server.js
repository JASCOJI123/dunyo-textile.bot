...
// Foydalanuvchi ilgari o'ynaganmi - tekshiradi
function findEntry(data, uid) {
  return data.entries.find(e => e.user_id === uid);
}
...
app.post('/webhook', async (req, res) => {
  ...
  if (msg.contact) {
    ...
    const already = findEntry(data, uid);
    if (already) {
      await tgApi('sendMessage', {
        chat_id: chatId,
        text: `Siz allaqachon o'ynagansiz 🎯\nYutug'ingiz: ${already.prize}${already.code ? '\nKod: ' + already.code : ''}\n\nHar bir mijoz faqat bir marta ishtirok eta oladi.`
      });
      return;
    }
  ...
  if (msg.web_app_data) {
    ...
    // MUHIM: bitta foydalanuvchi faqat bitta marta yutishi mumkin.
    // Bu tekshiruv serverda bo'lgani uchun, mini-ilovani qayta ochib
    // "aldab" bo'lmaydi.
    const already = findEntry(data, uid);
    if (already) {
      await tgApi('sendMessage', {
...
// Mini-ilova ochilganda, foydalanuvchi ilgari o'ynaganmi tekshirish uchun
// (o'yin sahifasi shu manzilga so'rov yuboradi va agar o'ynagan bo'lsa
// barabanni bloklaydi)
app.get('/check', (req, res) => {
  ...
  if (entry) {
    res.json({ played: true, prize: entry.prize, code: entry.code, type: entry.type });
  } else {
